...
enum ScannerLayoutMode { compact, fullscreen }
...
class ScanningPreferences extends ChangeNotifier {
  ...
  static const _keyScannerLayout = 'scan_pref_scanner_layout';
  ...
  ScannerLayoutMode _scannerLayout = ScannerLayoutMode.compact;
  ...
  ScannerLayoutMode get scannerLayout => _scannerLayout;
  ...
  static Future<ScanningPreferences> load() async {
    ...
    final layoutRaw = prefs.getString(_keyScannerLayout);
    instance._scannerLayout = ScannerLayoutMode.values.firstWhere(
      (e) => e.name == layoutRaw,
      orElse: () => ScannerLayoutMode.compact,
    );
  ...
  Future<void> setScannerLayout(ScannerLayoutMode v) async {
    _scannerLayout = v;
    notifyListeners();
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(_keyScannerLayout, v.name);
  }
}
